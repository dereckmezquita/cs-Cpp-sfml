...
We can draw a shape, give it a width and height, give it a texture, transperency etc. 

So first we need to create the shape object. When doing this we have multiple kinds possible:

```Cpp
sf::RectangleShape player(sf::Vector2f(100.0f, 100.0f));
```

Creating the object is the first step. Now we need to draw the object on the screen.
...
The way this works is that we have essentially two screens. These are buffers, one is displayed on top and the other in the back so to speak. What occurs when we display something on the screen is taht we draw to the back buffer, then when we want to show it, the back buffer switches places with the front buffer. This is to avoid issues where a screen is not completely done drawing; this is the reason we need to draw"" two separate times.


<p align="center">
    <img src="./figures/draw-square.png" width="100%">
</p>

If we want to add a colour we can do the following: `player.setFillColor(sf::Color::Red);`.

All together this code would look something like this:
...
<p align="center">
    <img src="./figures/draw-red-square.png" width="100%">
</p>
